...
import {
  GroupFilterParams,
  GetAllGroupsRequestBody,
  GroupProduct,
  ViewGroupProductsRequest,
} from '../types/GroupModels';
...
export class GroupAPI {
  ...
  async getAllGroups(params?: {
    ...
    try {
      const groupParams: GroupFilterParams | undefined =
        params?.fromDate || params?.updatedDate
          ? {
      ...
      const response = await this.http.post<{ groups?: string[] }>(
        `groups/${this.apiKey}/`,
        request
      );
  ...
  async getGroupProducts(groupName: string): Promise<GroupProduct[]> {
    try {
      ...
      const response = await this.http.post<{ products?: GroupProduct[] }>(
        `groups/${this.apiKey}/view`,
        request
      );
